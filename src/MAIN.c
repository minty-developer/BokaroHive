...
#define GAME_SET_CONSTANT Q1JFQVRFIEJZIE1JTlRZ
...
void CraftItem(CraftRecipe recipe)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        if (MyBag[i].item_id == recipe.material1 && MyBag[i].count >= recipe.count1)
            hasMat1 = true;
        if (recipe.material2 != ITEM_BLANK && MyBag[i].item_id == recipe.material2 && MyBag[i].count >= recipe.count2)
            hasMat2 = true;
    }
    ...
    if (hasMat1 && hasMat2)
    {
        ...
        if (recipe.material2 != ITEM_BLANK)
            BAG(BAG_SUB, recipe.material2, recipe.count2);
...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        {
            if (state_screen == SCREEN_GAMES_MAIN)
                state_screen = SCREEN_MENU;
            else if (state_screen >= SCREEN_SETTINGS)
                state_screen = SCREEN_GAMES_MAIN;
        }
        ...
        case SCREEN_MENU:
            ...
            {
                pointer_position--;
                ToAbleValue(1, 3, &pointer_position);
            }
        ...
        case SCREEN_GAMES_MAIN:
            if (IsKeyPressed(KEY_B))
                state_screen = SCREEN_GAMES_MAIN_BAG;
            if (IsKeyPressed(KEY_C))
                state_screen = SCREEN_GAMES_MAIN_CREATE;
            if (IsKeyPressed(KEY_A))
                state_screen = SCREEN_GAMES_MAIN_ADVENTURE;
            if (IsKeyPressed(KEY_S))
                SaveGame();
            break;
        ...
        case SCREEN_GAMES_MAIN_CREATE:
            ...
            {
                recipe_pointer--;
                ToAbleValue(0, TotalRecipes - 1, &recipe_pointer);
            }
            if (IsKeyPressed(KEY_S))
            {
                recipe_pointer++;
            ...
            if (IsKeyPressed(KEY_ENTER))
                CraftItem(RecipeDatabase[recipe_pointer]);
            break;
        ...
        case SCREEN_GAMES_MAIN:
            if (leekTexture.id > 0)
                DrawTextureEx(leekTexture, (Vector2){540, 200}, 0, 5.0f, WHITE);
        ...
        case SCREEN_GAMES_MAIN_ADVENTURE:
            ...
            if (isAdventuring)
            {
                    ...
                    if (logTimer >= 1.5f)
                    {
                        ...
                        { // 40% 확률로 돌발 이벤트
                            ...
                            if (activeEventType == 0)
                                AddLog("!! 경고: 데이터 폭풍 발생! [L]회피 [R]강행");
                            else
                                AddLog("!! 발견: 버려진 상자! [L]무시 [R]조사");
                        }
                        else
                        {
                            // 기존 일반 로그
                ...
                else
                {
                    ...
                    else if (IsKeyPressed(KEY_R))
                    {
                        if (activeEventType == 0)
                        {
                            ...
                            {
                                AddLog("-> 강행 성공! 데이터 획득.");
                                BAG(BAG_ADD, ITEM_MIKU_FRAGMENT, 2);
                            }
                            else
                            {
                                AddLog("-> 폭풍에 휘말려 동력 손실!");
                                playerHP -= 10;
                            }
                        }
                        else
                        {
                            AddLog("-> 상자에서 재료를 발견했습니다!");
                            BAG(BAG_ADD, ITEM_RUSTY_METAL, 1);
                        }
                        eventActive = false;
                    }
                ...
                if (logTimer >= 1.2f)
                {
                    ...
                    if (event == 0)
                        AddLog("..노이즈 섞인 노랫소리가 들립니다.");
                    else if (event == 1)
                        AddLog("..데이터 파편들이 휘몰아칩니다!");
                    else if (event == 2)
                        AddLog("..오래된 서버 렉을 발견했습니다.");
                    else
                        AddLog("..주변을 정밀 스캔 중...");
                }
